...
from internvl.model.internvlu import (InternVisionConfig,
                                      InternVisionModel,
                                      InternVLUChatConfig,
                                      InternVLUChatModel)
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        # v1: disable Qwen2-VL/NaViT-style anyres path; rely on dynamic_image_size only.
        ...
        logger.info('Building InternVLUChatConfig...')
        internvl_chat_config = InternVLUChatConfig(
        ...
        # v1: disable Qwen2-VL/NaViT-style anyres path; rely on dynamic_image_size only.
        internvl_chat_config.anyres_image_size = False
        logger.info('Building InternVLUChatModel...')
        model = InternVLUChatModel(internvl_chat_config, vision_model, llm)
    model.img_context_token_id = img_context_token_id

    # Register the U special tokens with the tokenizer and bind their token IDs onto the
    # model so replace_img_special_tokens() can swap LLM embeddings for the rows in
    # model.special_token_embedding. Do NOT call resize_token_embeddings for these — the
    # embeddings live in model.special_token_embedding, not the LLM table.
    tokenizer.add_special_tokens(
        {'additional_special_tokens': list(SPECIAL_TOKEN_LIST)}
    )
    model.special_token_id_list = [
        tokenizer.convert_tokens_to_ids(t) for t in model.special_token_list
    ]
    ...
    # The U-special-token embedding table is always trained, regardless of any other
    # freeze flag. Keep this AFTER every other freeze/unfreeze branch above.
